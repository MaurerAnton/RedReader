...
class OKHTTPBackend private constructor() : HTTPBackend() {
		...
		return object : Request {
			override fun executeInThisThread(listener: Listener) {
				...
				try {
					...
					RateLimitTracker.trackResponse(response)
	...
	companion object {
		...
		object RateLimitTracker {
			...
			var remaining = 600
				private set
			var resetSeconds = 600
				private set

			fun trackResponse(response: okhttp3.Response) {
				requestCount++
				lastResetTime = System.currentTimeMillis()

				response.header("X-Ratelimit-Remaining")
					?.toFloatOrNull()?.let { remaining = it.toInt() }

				response.header("X-Ratelimit-Reset")
					?.toIntOrNull()?.let { resetSeconds = it }
			}

			fun requestsPerMinute(): Int {
				val now = System.currentTimeMillis()
				val elapsedMinutes = (now - lastResetTime) / 60000.0
				return if (elapsedMinutes > 0 && requestCount > 0) {
					(requestCount / elapsedMinutes).toInt()
				} else {
					requestCount
				}
			}
