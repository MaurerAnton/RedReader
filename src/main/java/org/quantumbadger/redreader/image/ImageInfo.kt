	...
	companion object {
		...
		fun parseGfycat(obj: JsonObject): ImageInfo {
			...
			val original = obj.getString("mp4Url")?.let { originalUrl ->
				ImageUrlInfo(
					url = stripImageUrlParams(UriString(originalUrl)),
			...
			val preview = mobilePoster?.getString("url")?.let { previewUrl ->
				ImageUrlInfo(
					url = stripImageUrlParams(UriString(previewUrl)),
		...
		fun parseStreamable(obj: JsonObject): ImageInfo {
			...
			val original = fileObj.getString("url")?.let { originalUrl ->
				ImageUrlInfo(
					url = stripImageUrlParams(UriString(
		...
		fun parseImgur(obj: JsonObject?): ImageInfo {
			...
			val bigSquare = links.getString("big_square")?.let { bigSquareUrl ->
				ImageUrlInfo(url = stripImageUrlParams(UriString(bigSquareUrl)))
			}
		...
		fun parseImgurV3(obj: JsonObject?): ImageInfo {
			...
			val original = obj?.getString("mp4")?.takeIf { it.isNotEmpty() }?.let { urlMp4 ->
				...
				ImageUrlInfo(
					url = stripImageUrlParams(UriString(urlMp4)),
					size = originalSize,
				...
				obj?.getString("link")?.takeIf { it.isNotEmpty() }?.let { urlLink ->
					ImageUrlInfo(
						url = stripImageUrlParams(UriString(urlLink)),
						size = originalSize,
			...
			val bigSquare = obj?.getString("id")?.let {
				ImageUrlInfo(url = stripImageUrlParams(UriString("https://i.imgur.com/${it}b.jpg")))
			}
		...
		fun parseDeviantArt(obj: JsonObject?): ImageInfo {
			...
			val original = obj?.getString("url")?.let { url ->
				ImageUrlInfo(
					url = stripImageUrlParams(UriString(url)),
					size = ImageSize.fromJson(obj),
				)
			}

			val bigSquare = obj?.getString("thumbnail_url")?.let { url ->
				ImageUrlInfo(url = stripImageUrlParams(UriString(url)))
			}
		...
		fun parseRedgifsV2(obj: JsonObject): ImageInfo {
			...
			val original = obj.getStringAtPath("urls", "hd")
				...
				.asNullable()?.let {
					ImageUrlInfo(
						url = stripImageUrlParams(UriString(it)),
						size = ImageSize.fromJson(obj)
					)
				}
		...
		@JvmStatic
		fun stripImageUrlParams(url: UriString): UriString {
			if (!PrefsUtility.pref_images_strip_url_params()) return url

			val decoded = url.value.lowercase()
			if (decoded.contains("?") && (
						...
						|| decoded.endsWith(".bmp")
						|| decoded.contains("i.redd.it")
						|| decoded.contains("preview.redd.it")
						)) {
				val qPos = url.value.indexOf('?')
				if (qPos > 0) return UriString(url.value.substring(0, qPos))
			}
			return url
		}
	}
